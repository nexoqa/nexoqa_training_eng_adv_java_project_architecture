...
public class createTasks {
    ...
    public  List<taskModel> createSeveralTasks(int numTasks) throws SQLException {
        ...
        for (int i = 0; i < numTasks; ++i) {
            String randomTittle = RandomStringUtils.randomAlphanumeric(8).toUpperCase();
            String randomDescription = RandomStringUtils.randomAlphanumeric(15).toUpperCase();
            Timestamp timestamp = new Timestamp(System.currentTimeMillis());
            taskModel task = new taskModel(randomTittle, randomDescription);
            tasks.add(task);
            todoListAdapter.createTask(randomTittle, randomDescription, timestamp);
        }
        LOGGER.info(numTasks + " tasks created");
        return tasks;
    }
}
